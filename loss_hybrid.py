...
def batch_soft(dists, pids, margin, batch_precision_at_k=None):
    ...
    with tf.name_scope("batch_soft"):
        ...
        positive_mask = tf.logical_xor(same_identity_mask,
                                       tf.eye(tf.shape(pids)[0], dtype=tf.bool))
        me, var=tf.nn.moments(dists*tf.cast(positive_mask, tf.float32), axes=1)
        furthest_positive = tf.contrib.distributions.percentile(dists*tf.cast(positive_mask, tf.float32), axis=1, q=50)
        closest_negative = tf.map_fn(lambda x: tf.contrib.distributions.percentile(tf.boolean_mask(x[0], x[1]), q=50),(dists, negative_mask), tf.float32)
