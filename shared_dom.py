...
def main():
    ...
    toshow = tf.clip_by_value(t=imClip,clip_value_min=0,clip_value_max=255.0, name='clipRelu')#tf.cast(,tf.uint8)  
    toshowOrg = tf.cast(tf.clip_by_value(t=images,clip_value_min=0,clip_value_max=255.0),tf.uint8)  


    decDense2 = tf.layers.dense(inputs = endpoints['emb'],units=1280,name='decDense2')  #  ,activation = tf.nn.relu  ################
    unflat2 = tf.reshape(decDense2,shape = [tf.shape(decDense2)[0],16,8,10])
    unp3shape2 = tf.TensorShape([2*di for di in unflat2.get_shape().as_list()[1:-1]])
    unPool32 = tf.image.resize_nearest_neighbor(unflat2,unp3shape2 , name = 'unpool32')
    
    deConv32 = tf.layers.conv2d(inputs = unPool32,filters = 64,kernel_size = [5,5],strides=(1, 1),padding='same',activation=tf.nn.relu,name='deConv32')
    unp2shape2 = tf.TensorShape([2*di for di in deConv32.get_shape().as_list()[1:-1]])
    unPool22 = tf.image.resize_nearest_neighbor(deConv32, unp2shape2,name = 'unpool22')
    print('a',deConv32.shape)
    deConv22 = tf.layers.conv2d(inputs = unPool22,filters = 32,kernel_size = [5,5],strides=(1, 1),padding='same',activation=tf.nn.relu,name='deConv22')
    unp1shape2 = tf.TensorShape([2*di for di in deConv22.get_shape().as_list()[1:-1]])
    unPool12 = tf.image.resize_nearest_neighbor(deConv22, unp1shape2, name = 'unpool12')
    print('k',deConv22.shape)
    deConv12 = tf.layers.conv2d(inputs = unPool12,filters = 3,kernel_size = [5,5],strides=(1, 1),padding='same',activation=None,name='deConv12')
    imClip1 = deConv12
    ...
    decDense3 = tf.layers.dense(inputs = endpoints['emb'],units=320,name='decDense3')  #  ,activation = tf.nn.relu  ################
    unflat23 = tf.reshape(decDense3,shape = [tf.shape(decDense3)[0],8,4,10])
    unp3shape23 = tf.TensorShape([2*di for di in unflat23.get_shape().as_list()[1:-1]])
    unPool323 = tf.image.resize_nearest_neighbor(unflat23,unp3shape23 , name = 'unpool323')

    deConv323 = tf.layers.conv2d(inputs = unPool323,filters = 64,kernel_size = [5,5],strides=(1, 1),padding='same',activation=tf.nn.relu,name='deConv323')
    unp2shape23 = tf.TensorShape([2*di for di in deConv323.get_shape().as_list()[1:-1]])
    unPool223 = tf.image.resize_nearest_neighbor(deConv323, unp2shape23,name = 'unpool223')
    print(deConv323.shape)
    deConv223 = tf.layers.conv2d(inputs = unPool223,filters = 32,kernel_size = [5,5],strides=(1, 1),padding='same',activation=tf.nn.relu,name='deConv223')
    unp1shape23 = tf.TensorShape([2*di for di in deConv223.get_shape().as_list()[1:-1]])
    unPool123 = tf.image.resize_nearest_neighbor(deConv223, unp1shape23, name = 'unpool123')
    print(deConv223.shape)
    deConv123 = tf.layers.conv2d(inputs = unPool123,filters = 3,kernel_size = [5,5],strides=(1, 1),padding='same',activation=None,name='deConv123')
    imClip2 = deConv123
    ...
    with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
        train_op = optimizer.minimize(tf.add(loss_mean,tf.add(recLoss/(255.0*255.0),tf.add(recLoss2/(255.0*255.0),recLoss3/(255.0*255.0)))), global_step=global_step)
        train_op1 = optimizer.minimize(tf.add(loss_mean1,tf.add(recLoss/(255.0*255.0),tf.add(recLoss2/(255.0*255.0),recLoss3/(255.0*255.0)))), global_step=global_step)
