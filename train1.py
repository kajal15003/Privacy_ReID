...
parser.add_argument(
    '--loss', default='batch_soft', choices=loss.LOSS_CHOICES.keys(),
...
def main():
    ...
    toshow = tf.cast(tf.clip_by_value(t=deConv1,clip_value_min=0,clip_value_max=255.0, name='clipRelu'),tf.uint8)
    ...
    tf.summary.image('Batchimage', toshow, max_outputs=2)
    tf.summary.image('Batchimage', toshowOrg, max_outputs=2)
    ...
    with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
        train_op = optimizer.minimize(tf.add(loss_mean,tf.add(recLoss,recLoss1)), global_step=global_step)
