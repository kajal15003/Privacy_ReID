...
parser.add_argument(
    '--experiment_root', required=True, type=commoncuhk.writeable_directory,
...
parser.add_argument(
    '--image_root', type=commoncuhk.readable_directory,
...
parser.add_argument(
    '--embedding_dim', default=128, type=commoncuhk.positive_int,
...
parser.add_argument(
    '--batch_p', default=32, type=commoncuhk.positive_int,
    help='The number P used in the PK-batches')

parser.add_argument(
    '--batch_k', default=4, type=commoncuhk.positive_int,
    help='The numberK used in the PK-batches')

parser.add_argument(
    '--net_input_height', default=256, type=commoncuhk.positive_int,
    help='Height of the input directly fed into the network.')

parser.add_argument(
    '--net_input_width', default=128, type=commoncuhk.positive_int,
    help='Width of the input directly fed into the network.')

parser.add_argument(
    '--pre_crop_height', default=288, type=commoncuhk.positive_int,
    help='Height used to resize a loaded image. This is ignored when no crop '
         'augmentation is applied.')

parser.add_argument(
    '--pre_crop_width', default=144, type=commoncuhk.positive_int,
    help='Width used to resize a loaded image. This is ignored when no crop '
         'augmentation is applied.')
# TODO end

parser.add_argument(
    '--loading_threads', default=8, type=commoncuhk.positive_int,
    help='Number of threads used for parallel loading.')

parser.add_argument(
    '--margin', default='soft', type=commoncuhk.float_or_string,
...
parser.add_argument(
    '--learning_rate', default=3e-4, type=commoncuhk.positive_float,
    help='The initial value of the learning-rate, before it kicks in.')

parser.add_argument(
    '--train_iterations', default=25000, type=commoncuhk.positive_int,
...
parser.add_argument(
    '--checkpoint_frequency', default=1000, type=commoncuhk.nonnegative_int,
...
def main():
    ...
    logging.config.dictConfig(commoncuhk.get_logging_dict(log_file))
    ...
    pids, fids = commoncuhk.load_dataset(args.train_set, args.image_root)
    ...
    dataset = dataset.map(
        lambda fid, pid: commoncuhk.fid_to_image(
    ...
    pred = tf.layers.dense(inputs = endpoints['emb'],units=128,name='ce',activation=tf.nn.softmax)
    labels =tf.eye(128)
    ce_loss = tf.losses.softmax_cross_entropy(tf.convert_to_tensor(labels),pred,weights=0.5, label_smoothing=0.2)
    ...
    images2=images + np.random.laplace(0,1)

    recLoss = tf.losses.mean_squared_error(labels = images2,predictions = imClip)
    ...
    tf.summary.scalar('ce_loss', ce_loss_mean)
    ...
    with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
        train_op = optimizer.minimize(tf.add(tf.add(loss_mean,ce_loss),recLoss/(255.0*255.0)), global_step=global_step)
    ...
    with tf.Session() as sess:
        ...
        with lb.Uninterrupt(sigs=[SIGINT, SIGTERM], verbose=True) as u:
            for i in range(start_step, args.train_iterations):
                ...
                _, summary, step, b_prec_at_k, b_embs, b_loss, b_fids ,b_rec,ce_los= \
                    sess.run([train_op, merged_summary, global_step,
                              prec_at_k, endpoints['emb'], losses, fids,recLoss,ce_loss])
                ...
                log.info('iter:{:6d}, loss min|avg|max: {:.3f}|{:.3f}|{:6.3f}, '
                         'celoss:{:.2f} recLoss: {:.3f} batch-p@{}: {:.2%}, ETA: {} ({:.2f}s/it)'.format(
                             step,
                             float(np.min(b_loss)),
