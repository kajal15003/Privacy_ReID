...
def main():
    ...
    decDense = tf.compat.v1.layers.dense(inputs = endpoints['emb'],units=80,name='decDense')  #  ,activation = tf.compat.v1.nn.relu  ################
    unflat = tf.compat.v1.reshape(decDense,shape = [tf.compat.v1.shape(decDense)[0],4,2,10])
    ...
    images2=tf.compat.v1.image.resize_images(images, [32, 16])
