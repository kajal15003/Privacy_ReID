...
def sample_k_fids_for_pid(pid, all_fids, all_pids, batch_k):
    ...
    possible_fids = tf.compat.v1.boolean_mask(all_fids, tf.compat.v1.equal(all_pids, pid))
    ...
    count = tf.compat.v1.shape(possible_fids)[0]
    padded_count = tf.compat.v1.cast(tf.compat.v1.ceil(batch_k / count), tf.compat.v1.int32) * count
    full_range = tf.compat.v1.mod(tf.compat.v1.range(padded_count), count)

    # Sampling is always performed by shuffling and taking the first k.
    shuffled = tf.compat.v1.random_shuffle(full_range)
    selected_fids = tf.compat.v1.gather(possible_fids, shuffled[:batch_k])

    return selected_fids, tf.compat.v1.fill([batch_k], pid)
...
def main():
    ...
    # Setup a tf.compat.v1.Dataset where one "epoch" loops over all PIDS.
    # PIDS are shuffled after every epoch and continue indefinitely.
    unique_pids = np.unique(pids)
    dataset = tf.compat.v1.data.Dataset.from_tensor_slices(unique_pids)
    ...
    if args.flip_augment:
        dataset = dataset.map(
            lambda im, fid, pid: (tf.compat.v1.image.random_flip_left_right(im), fid, pid))
    if args.crop_augment:
        dataset = dataset.map(
            lambda im, fid, pid: (tf.compat.v1.random_crop(im, net_input_size + (3,)), fid, pid))
    ...
    decDense = tf.compat.v1.layers.dense(inputs = endpoints['emb'],units=1280,name='decDense')  #  ,activation = tf.compat.v1.nn.relu  ################
    unflat = tf.compat.v1.reshape(decDense,shape = [tf.compat.v1.shape(decDense)[0],16,8,10])
    unp3shape = tf.compat.v1.TensorShape([2*di for di in unflat.get_shape().as_list()[1:-1]])
    unPool3 = tf.compat.v1.image.resize_nearest_neighbor(unflat,unp3shape , name = 'unpool3')

    deConv3 = tf.compat.v1.layers.conv2d(inputs = unPool3,filters = 64,kernel_size = [5,5],strides=(1, 1),padding='same',activation=tf.compat.v1.nn.relu,name='deConv3')
    unp2shape = tf.compat.v1.TensorShape([2*di for di in deConv3.get_shape().as_list()[1:-1]])
    unPool2 = tf.compat.v1.image.resize_nearest_neighbor(deConv3, unp2shape,name = 'unpool2')

    deConv2 = tf.compat.v1.layers.conv2d(inputs = unPool2,filters = 32,kernel_size = [5,5],strides=(1, 1),padding='same',activation=tf.compat.v1.nn.relu,name='deConv2')
    unp1shape = tf.compat.v1.TensorShape([2*di for di in deConv2.get_shape().as_list()[1:-1]])
    unPool1 = tf.compat.v1.image.resize_nearest_neighbor(deConv2, unp1shape, name = 'unpool1')

    deConv1 = tf.compat.v1.layers.conv2d(inputs = unPool1,filters = 3,kernel_size = [5,5],strides=(1, 1),padding='same',activation=None,name='deConv1')
    imClip = deConv1  #tf.compat.v1.clip_by_value(t = deConv1,clip_value_min = -1.0,clip_value_max = 1.0,name='clipRelu')
    print('RconstructeddImage :  ', imClip.name)

    images2=tf.compat.v1.image.resize_images(images, [128, 64])

    recLoss = tf.compat.v1.losses.mean_squared_error(
                labels = images2,
                predictions = imClip,
            )
    print('recLoss :  ', recLoss.name)
    toshow = tf.compat.v1.clip_by_value(t=imClip,clip_value_min=0,clip_value_max=255.0, name='clipRelu')#tf.compat.v1.cast(,tf.compat.v1.uint8)  
    toshowOrg = tf.compat.v1.cast(tf.compat.v1.clip_by_value(t=images,clip_value_min=0,clip_value_max=255.0),tf.compat.v1.uint8)  


    # Count the number of active entries, and compute the total batch loss.
    num_active = tf.compat.v1.reduce_sum(tf.compat.v1.cast(tf.compat.v1.greater(losses, 1e-5), tf.compat.v1.float32))
    loss_mean = tf.compat.v1.reduce_mean(losses)
    recMean = tf.compat.v1.reduce_mean(imClip)
    ...
    tf.compat.v1.summary.scalar('recImagemean', recMean)    
    ...
    tf.compat.v1.summary.histogram('embedding_lengths',tf.norm(endpoints['emb_raw'], axis=1))
    tf.compat.v1.summary.image('Batchimage', toshow, max_outputs=3)
    tf.compat.v1.summary.image('Batchimage', toshowOrg, max_outputs=3)
    ...
    if 0 <= args.decay_start_iteration < args.train_iterations:
        learning_rate = tf.compat.v1.train.exponential_decay(
            args.learning_rate,
            tf.compat.v1.maximum(0, global_step - args.decay_start_iteration),
    ...
    with tf.compat.v1.control_dependencies(tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS)):
        train_op = optimizer.minimize(tf.compat.v1.add(loss_mean,recLoss/(255.0*255.0)), global_step=global_step)
    ...
    with tf.compat.v1.Session() as sess:
        if args.resume:
            ...
            last_checkpoint = tf.compat.v1.train.latest_checkpoint(args.experiment_root)
