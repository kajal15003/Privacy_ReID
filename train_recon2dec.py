...
def main():
    ...
    decDense2 = tf.layers.dense(inputs = endpoints['emb'],units=80,name='decDense2')  #  ,activation = tf.nn.relu  ################
    unflat2 = tf.reshape(decDense2,shape = [tf.shape(decDense2)[0],4,2,10])
    ...
    images2=tf.image.resize_images(images, [32, 16])
    print(images2.shape)
    ...
    low_r = tf.clip_by_value(t=imClip1,clip_value_min=0,clip_value_max=255, name='clprel')  
    org_low = tf.cast(tf.clip_by_value(t=images2,clip_value_min=0,clip_value_max=255),tf.uint8)  
    ...
    recmean2=tf.reduce_mean(imClip1)
    ...
    tf.summary.scalar('recLos2', recLoss2)
    ...
    tf.summary.image('Batchimage', toshow, max_outputs=3)
    tf.summary.image('Batchimage', toshowOrg, max_outputs=3)
    tf.summary.image('Batchimage', low_r, max_outputs=3)
    tf.summary.image('Batchimage', org_low, max_outputs=3)
    ...
    with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
        train_op = optimizer.minimize(tf.add(loss_mean,tf.add(recLoss/(255.0*255.0),recLoss2/(255.0*255.0))),global_step=global_step)
    ...
    with tf.Session() as sess:
        ...
        with lb.Uninterrupt(sigs=[SIGINT, SIGTERM], verbose=True) as u:
            for i in range(start_step, args.train_iterations):
                ...
                _, summary, step, b_prec_at_k, b_embs, b_loss, b_fids ,b_rec,b_rec2= \
                    sess.run([train_op, merged_summary, global_step,
                              prec_at_k, endpoints['emb'], losses, fids,recLoss,recLoss2])
                ...
                log.info('iter:{:6d}, loss min|avg|max: {:.3f}|{:.3f}|{:6.3f}, '
                         'recLoss: {:.3f} recLoss: {:.3f} batch-p@{}: {:.2%},ETA: {} ({:.2f}s/it)'.format(
                             step,
                             float(np.min(b_loss)),
