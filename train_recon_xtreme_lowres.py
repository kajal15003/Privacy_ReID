...
def main():
    ...
    decDense = tf.layers.dense(inputs = endpoints['emb'],units=320,name='decDense')  #  ,activation = tf.nn.relu  ################
    unflat = tf.reshape(decDense,shape = [tf.shape(decDense)[0],8,4,10])
    ...
    images2=tf.image.resize_images(images, [64,32])
