...
def main():
    ...
    with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
        train_op = optimizer.minimize(tf.add(loss_mean,recLoss/(128.0*128.0)), global_step=global_step)
        train_op_2 = optimizer.minimize(loss_mean, global_step=global_step)
        ...
        with lb.Uninterrupt(sigs=[SIGINT, SIGTERM], verbose=True) as u:
            for i in range(start_step, args.train_iterations):
                ...
                _, summary, step, b_prec_at_k, b_embs, b_loss, b_fids ,b_rec= \
                    sess.run([t_op, merged_summary, global_step,
