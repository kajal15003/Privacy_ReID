...
def main():
    ...
    deConv3 = tf.layers.conv2d(inputs = unPool3,filters = 3,kernel_size = [5,5],strides=(1, 1),padding='same',activation=tf.nn.relu,name='deConv3')
    ...
    deConv2 = tf.layers.conv2d(inputs = unPool2,filters = 3,kernel_size = [5,5],strides=(1, 1),padding='same',activation=tf.nn.relu,name='deConv2')
    ...
    unflat2 = tf.reshape(decDense,shape = [tf.shape(decDense)[0],32,16,10])
    ...
    unPool22 = tf.image.resize_nearest_neighbor(deConv32, unp2shape,name = 'unpool22')

    deConv22 = tf.layers.conv2d(inputs = unPool22,filters = 3,kernel_size = [5,5],strides=(1, 1),padding='same',activation=tf.nn.relu,name='deConv22')
    ...
    unflat23 = tf.reshape(decDense,shape = [tf.shape(decDense)[0],32,16,10])
    ...
    deConv323 = tf.layers.conv2d(inputs = unPool323,filters = 3,kernel_size = [5,5],strides=(1, 1),padding='same',activation=tf.nn.relu,name='deConv323')
    #unp2shape23 = tf.TensorShape([2*di for di in deConv323.get_shape().as_list()[1:-1]])
    #unPool223 = tf.image.resize_nearest_neighbor(deConv323, unp2shape,name = 'unpool223')
