...
def main():
    ...
    ##Dense layers for u and v
    Dense_u = tf.layers.dense(inputs = endpoints['emb'],units=128,name='Dense_u',activation =tf.nn.relu) 
    Dense_v = tf.layers.dense(inputs = endpoints['emb'],units=128,name='Dense_v',activation =tf.nn.relu) 
    #predicts = Dense_v
    #print(pids,pids.shape,predicts.shape)
    
    #cls_loss=tf.losses.sigmoid_cross_entropy(multi_class_labels=pids, logits=predicts)
    #cls_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=pids, logits=predicts)) 
        
    # Create the loss in two steps:
    # 1. Compute all pairwise distances according to the specified metric.
    # 2. For each anchor along the first dimension, compute its loss.
    dists = loss.cdist(Dense_u, Dense_u, metric=args.metric)
    ...
    with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
        train_op = optimizer.minimize(tf.add(loss_mean,dis_mean), global_step=global_step)
    ...
    with tf.Session() as sess:
        ...
        with lb.Uninterrupt(sigs=[SIGINT, SIGTERM], verbose=True) as u:
            for i in range(start_step, args.train_iterations):
                ...
                _, summary, step, b_prec_at_k, b_embs, b_loss, b_fids, disl = \
                    sess.run([train_op, merged_summary, global_step,
                              prec_at_k, Dense_u, losses, fids, dis_loss])
                ...
                log.info('iter:{:6d}, loss min|avg|max: {:.3f}|{:.3f}|{:6.3f}, '
                             'dis_Loss: {:.3f}, batch-p@{}: {:.2%}, ETA: {} ({:.2f}s/it)'.format(
                             step,
                             float(np.min(b_loss)),
