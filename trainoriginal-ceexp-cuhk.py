...
parser.add_argument(
    '--checkpoint_frequency', default=5000, type=commoncuhk.nonnegative_int,
...
def main():
    ...
    # prefix.
    endpoints, body_prefix = model.endpoints(images, is_training=True, num_class=len(unique_pids))
    ...
    dists = loss.cdist(endpoints['emb']+0.01, endpoints['emb']+0.01, metric=args.metric)
    ...
    predictions = tf.layers.dense(inputs = endpoints['emb'],units=128,name='decDense',activation=tf.nn.softmax)
    labels =tf.eye(128)
    ce_loss = tf.losses.softmax_cross_entropy(tf.convert_to_tensor(labels), predictions,weights=0.5, label_smoothing=0.2)
    ...
    with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
        train_op = optimizer.minimize(tf.add(loss_mean, ce_loss),global_step=global_step)
    ...
    with tf.Session() as sess:
        ...
        with lb.Uninterrupt(sigs=[SIGINT, SIGTERM], verbose=True) as u:
            for i in range(start_step, args.train_iterations):
                ...
                _, summary, step, b_prec_at_k, b_embs, b_loss, b_fids,ce_los = \
                    sess.run([train_op, merged_summary, global_step,
                              prec_at_k, endpoints['emb'], losses, fids, ce_loss])
                ...
                log.info('iter:{:6d}, loss min|avg|max: {:.3f}|{:.3f}|{:6.3f}, '
                         ' ce_loss: {:.2%}, batch-p@{}: {:.2%}, ETA: {} ({:.2f}s/it)'.format(
                             step,
                             float(np.min(b_loss)),
