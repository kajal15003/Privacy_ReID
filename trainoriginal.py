...
def main():
 ...
 #   for i in fids:
  #      par = i[19:23]
   #     tt = [1 if i == par else 0 for i in pidf]
    #    labels.append(tt)
    ce_loss = tf.losses.softmax_cross_entropy(tf.convert_to_tensor(labels), predictions,
                                                                weights=0.5, label_smoothing=0.2)
    #predictions=tf.contrib.layers.softmax(decDense)
    #print(predictions,unique_pids.shape)
    #cross_ent = tf.losses.softmax_cross_entropy(pids,predictions,weights=0.5, label_smoothing=0.2)
    #print(cross_ent)
